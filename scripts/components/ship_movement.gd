...
@export var turn_acceleration: float = 10
...
func _physics_process(delta: float) -> void:
	...
	if turn_input:
		var inverse_intertia := PhysicsServer3D.body_get_direct_state(_parent.get_rid()).inverse_inertia.y
		var acceleration_defficit := (max_turn_speed - absf(_parent.angular_velocity.y)) / delta
		var torque_to_apply: float = turn_input * clamp(turn_acceleration, -acceleration_defficit, acceleration_defficit) / inverse_intertia
